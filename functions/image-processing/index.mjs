...
export const handler = async (event) => {
    ...
    /**
     * An example of expected path is /images/rio/1.jpeg/format=auto,width=100 or
     * /images/rio/1.jpeg/original where
     * /images/rio/1.jpeg is the path of the original image
     */
     ...
     */
    const operationPrefix = imagePathArray.pop();
     ...
     */
    const operationsJSON = Object.fromEntries(operationPrefix.split(',').map(operation => operation.split('=')));
    ...
    if (S3_TRANSFORMED_IMAGE_BUCKET) {
        ...
        try {
            const putImageCommand = new PutObjectCommand({
                ...
                Key: originalImagePath + '/' + operationPrefix,
            ...
            if (imageTooBig) {
                return {
                    statusCode: 302,
                    headers: {
                        'Location': '/' + originalImagePath + '?' + operationPrefix.replace(/,/g, "&"),
